...
echo.
echo   Sign in with accounts stored in your database.
...
REM ---------------------------------------------------- failures
:database_failed
echo  [X] Database initialization/migration failed.
echo      The schema must be created before the first Admin account.
...
:admin_failed
echo  [X] Admin setup failed. Check the email/password entered or the RPR_ADMIN_* variables.
echo.
pause
exit /b 1
