...
class User(Base):
    ...
    role = Column(String(20), default="admin")  # admin | driver | customer
...
class Customer(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), index=True)   # login account
...
class LoadRequest(Base):
    ...
    accepted_driver_id = Column(Integer, ForeignKey("drivers.id"))
    accepted_truck_id = Column(Integer, ForeignKey("trucks.id"))
...
class LoadMatch(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    load_request_id = Column(Integer, ForeignKey("load_requests.id"), nullable=False)
...
class Payment(Base):
    ...
    payment_mode = Column(String(20), default="UPI")     # UPI | Cash | Bank Transfer | Online
