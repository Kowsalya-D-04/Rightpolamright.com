...
from services.geo import resolve_point, road_distance_km, google_route_info
...
def _load_out(l: LoadRequest) -> dict:
    ...
    return {
        ...
        "expected_delivery_date": l.expected_delivery_date, "contact_number": l.contact_number,
...
def book_load(payload: CustomerLoadCreate, db: Session = Depends(get_db),
    ...
    route = google_route_info(p[0], p[1], d[0], d[1])
    distance = route["distance_km"] if route else road_distance_km(p[0], p[1], d[0], d[1]) if p[0] and d[0] else None
    ...
    load = LoadRequest(
        ...
        expected_delivery_date=payload.expected_delivery_date, contact_number=payload.contact_number or c.phone,
...
def payments(db: Session = Depends(get_db), user=Depends(require_customer)):
    ...
    for p in rows:
        ...
        out.append({
            ...
            "payment_mode": p.payment_mode, "transaction_reference": p.transaction_reference, "created_at": p.created_at,
            "paid_at": p.paid_at,
...
def initiate_payment(payment_id: int, payload: dict, db: Session = Depends(get_db), user=Depends(require_customer)):
    """Create a real payment intent state; never fabricates a successful gateway response."""
    import uuid
    c = _me(db, user)
    p = db.query(Payment).filter(Payment.id == payment_id, Payment.customer_id == c.id).first()
    if not p: raise HTTPException(404, "Payment not found on your account.")
    if p.status in ("SUCCESS", "FULLY_PAID", "Paid"):
        raise HTTPException(400, "This payment is already complete.")
    mode=(payload.get("payment_mode") or "").strip()
    if mode not in ("UPI", "Card", "Net Banking", "Wallet", "Cash"):
        raise HTTPException(400, "Choose UPI, Card, Net Banking, Wallet or Cash.")
    p.payment_mode=mode
    p.transaction_reference=p.transaction_reference or ("RPR-"+uuid.uuid4().hex[:16].upper())
    p.status="PENDING" if mode == "Cash" else "INITIATED"
    db.commit(); db.refresh(p)
    return {"id":p.id,"code":p.code,"status":p.status,"payment_mode":p.payment_mode,"transaction_reference":p.transaction_reference,"message":("Cash payment is pending collection." if mode=="Cash" else "Payment intent created. Complete payment through the configured payment provider; success is never assumed.")}
