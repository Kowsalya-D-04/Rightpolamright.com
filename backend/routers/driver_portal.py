...
def _trip_out(t: Trip) -> dict:
    ...
    return {
        ...
        "delivery_photo_url": t.delivery_photo_url, "delivery_signature_url": t.delivery_signature_url,
        "delivery_confirmed_at": t.delivery_confirmed_at,
...
def update_trip_status(trip_id: int, payload: TripStatusUpdate,
    ...
    if payload.status == "Delivered" and (not t.delivery_photo_url or not t.delivery_signature_url):
        raise HTTPException(400, "Upload delivery photo and customer signature before marking Delivered.")
...
@router.post("/trips/{trip_id}/proof-of-delivery")
def proof_of_delivery(trip_id: int, payload: dict, db: Session = Depends(get_db), user=Depends(require_driver)):
    d=_me(db,user)
    t=db.query(Trip).filter(Trip.id==trip_id,Trip.driver_id==d.id).first()
    if not t: raise HTTPException(404,"Trip not found on your account.")
    photo=(payload.get("delivery_photo_url") or "").strip(); signature=(payload.get("delivery_signature_url") or "").strip()
    if not photo or not signature: raise HTTPException(400,"Delivery photo and customer signature are required.")
    t.delivery_photo_url=photo; t.delivery_signature_url=signature; t.delivery_confirmed_at=datetime.now()
    db.commit()
    return {"trip_id":t.id,"proof_saved":True,"delivery_confirmed_at":t.delivery_confirmed_at}
...
def add_availability(payload: AvailabilitySlotCreate, db: Session = Depends(get_db),
    ...
    if point[0] is None:
        raise HTTPException(
            400, "Choose the starting location from Google Places so coordinates are saved.")
    ...
    slot = DriverAvailabilitySlot(
        ...
        trip_type=payload.trip_type or "NEW_LOAD",
        total_capacity_ton=payload.total_capacity_ton or truck.capacity_ton,
        available_capacity_ton=payload.available_capacity_ton if payload.available_capacity_ton is not None else truck.capacity_ton,
