...
from models import AuditLog, PricingRule, RoutePricing
...
def update_rule(rule_id: int, payload: PricingRuleUpdate, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if not reason:
        raise HTTPException(400, "A reason is required for every pricing override/change.")
    import json
    before = {c.name: getattr(rule, c.name) for c in PricingRule.__table__.columns}
    for field in ("base_fare", "rate_per_km", "rate_per_ton", "fuel_charge_per_km", "loading_charge",
    ...
    for k, v in data.items():
        setattr(rule, k, v)
    db.flush()
    after = {c.name: getattr(rule, c.name) for c in PricingRule.__table__.columns}
    db.add(AuditLog(actor_user_id=user.id, action="PRICING_RULE_UPDATED", entity_type="PricingRule", entity_id=rule.id, reason=reason, before_json=json.dumps(before, default=str), after_json=json.dumps(after, default=str)))
    db.commit()
    db.refresh(rule)
...
def quote(payload: QuoteRequest, db: Session = Depends(get_db)):
    ...
    distance = route["distance_km"] if route else road_distance_km(p[0], p[1], d[0], d[1])
    ...
    return {
        ...
        "duration_minutes": route.get("duration_minutes") if route else None,
        "duration_text": route.get("duration_text") if route else None,
    }
