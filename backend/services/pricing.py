...
DEFAULT_RULE = dict(
    base_fare=8000, rate_per_km=20, rate_per_ton=300, loading_charge=1000,
...
def _rule_for(db: Session, truck_type: str) -> dict:
    ...
    return dict(
        ...
        loading_charge=rule.loading_charge, unloading_charge=rule.unloading_charge,
...
def calculate_fare(
    ...
    weight_charge = round((weight_ton or 0) * rule["rate_per_ton"])
    ...
    subtotal_before_fees = (
        base_fare + distance_charge + weight_charge + toll + loading + unloading + bata
    )
