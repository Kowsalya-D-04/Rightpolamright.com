...
from models import (
    Driver, DriverAvailability, DriverAvailabilitySlot, LoadMatch, LoadRequest,
    Trip, Truck,
)
...
WEIGHTS = {
    "capacity": 20,     # how well capacity fits without wasting a big truck
    "truck_type": 15,   # exact type match vs acceptable substitute
    "distance": 25,     # proximity of truck to pickup point
    "availability": 15, # driver + truck operational status
    "rating": 10,       # historical driver quality
    "date_time": 10,    # free on the requested date
    "route": 5,         # already positioned along the requested corridor
}
...
def find_matches(db: Session, load: LoadRequest, limit: int = 10, persist: bool = True):
    ...
    for truck in candidates:
        ...
        components = {
            "capacity": _capacity_score(truck.capacity_ton, load.weight_ton),
            "truck_type": 1.0 if truck.truck_type == load.truck_type else 0.7,
            "distance": _distance_score(distance),
            "availability": _availability_score(driver, truck),
            "rating": _rating_score(driver.rating),
            "date_time": date_component,
            "route": round((_route_score(load, driver) + preferred_route) / 2, 4),
        }
