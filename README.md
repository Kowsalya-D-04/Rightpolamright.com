...
**React + Vite** frontend · **FastAPI** backend · **SQLite** database (migrates cleanly to MySQL/PostgreSQL).
...
It creates the Python virtual environment, installs both dependency sets, seeds the demo
database on first run, and opens two terminal windows — one for the API, one for the web app.
...
Every seeded customer and driver shares the same demo password. You can also
register a brand-new customer or driver from the home page — admin accounts are
sign-in only and cannot be created from the site.

To wipe and re-seed the demo data, run **`reset-database.bat`**.
...
On macOS or Linux the only difference is `source venv/bin/activate`.
...
Only the connection string changes — no model code depends on SQLite:

```bat
set DATABASE_URL=postgresql+psycopg2://user:pass@localhost/rightpolamright
python seed.py
```

Install the matching driver (`psycopg2-binary` or `PyMySQL`) first.
