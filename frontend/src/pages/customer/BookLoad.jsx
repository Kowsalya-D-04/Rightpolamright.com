...
export default function BookLoad() {
  ...
  const [form, setForm] = useState({
    ...
    required_date: '', required_time: '09:00', expected_delivery_date: '', contact_number: '', special_instructions: '',
  })
  ...
  useEffect(() => {
    if (pickup.lat == null || pickup.lng == null || drop.lat == null || drop.lng == null) { setRouteInfo(null); return }
    const t=setTimeout(()=>{
      api.get('/locations/route', { params: { pickup_lat: pickup.lat, pickup_lng: pickup.lng, drop_lat: drop.lat, drop_lng: drop.lng } })
        .then(({data})=>setRouteInfo(data)).catch(()=>setRouteInfo(null))
    },250)
    return ()=>clearTimeout(t)
  ...
  const submit = async () => {
    ...
    try {
      await api.post('/customer/loads', {
        ...
        expected_delivery_date: form.expected_delivery_date || null, contact_number: form.contact_number || null,
            ...
            <div className="grid grid-2">
              ...
              <div className="field">
                ...
                <input className="input" value={form.contact_number}
                  onChange={set('contact_number')} placeholder="Defaults to your registered phone" />
              </div>
            </div>
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ padding: 13 }}>
              {markers.length === 2 ? (
                <>
                  <MapView height={230} markers={markers} route={{ from: { ...pickup }, to: { ...drop } }} />
                  {routeInfo?.duration_text && <div className="route-meta"><strong>Estimated travel:</strong> {routeInfo.duration_text}</div>}
                  {routeInfo?.route_towns?.length > 0 && <div className="route-towns"><strong>Route towns:</strong> {routeInfo.route_towns.join(' → ')}</div>}
                </>
              ) : (
                <div style={{
