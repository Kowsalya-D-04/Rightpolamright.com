...
export default function CustomerPayments() {
  ...
  const load=()=>api.get('/customer/payments').then(({ data }) => setData(data))
  useEffect(() => { load() }, [])
  const initiate=async(id)=>{ await api.post(`/customer/payments/${id}/initiate`,{payment_mode:mode}); await load() }
          ...
          <div className="table-wrap">
            <table className="data">
              <thead><tr><th>Payment ID</th><th>Trip</th><th>Route</th><th>Amount</th>
                <th>Mode</th><th>Date</th><th>Status</th><th>Action</th></tr></thead>
              <tbody>
                {data.payments.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{['Paid','SUCCESS','FULLY_PAID'].includes(p.status) ? '—' : <div style={{display:'flex',gap:6}}><select className="select" value={mode} onChange={(e)=>setMode(e.target.value)} style={{width:120}}><option>UPI</option><option>Card</option><option>Net Banking</option><option>Wallet</option><option>Cash</option></select><button className="btn btn-primary btn-sm" onClick={()=>initiate(p.id)}>Pay</button></div>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
