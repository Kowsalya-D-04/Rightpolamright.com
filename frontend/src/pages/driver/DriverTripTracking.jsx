...
function TripDetail({ tripId }) {
  ...
  /** Stands in for the driver app's GPS feed. */
  const ping = async () => {
    const cur = trip.current_lat ? { lat: trip.current_lat, lng: trip.current_lng }
      : { lat: trip.pickup_lat, lng: trip.pickup_lng }
    if (!cur.lat || !trip.drop_lat) return
    await api.post(`/driver/trips/${tripId}/location`, {
      lat: cur.lat + (trip.drop_lat - cur.lat) * 0.25,
      lng: cur.lng + (trip.drop_lng - cur.lng) * 0.25,
      speed_kmph: 52,
    })
    load()
  }
    ...
    <div className="page">
      <div className="page-head">
        ...
        <div className="page-actions">
          ...
          <button className="btn btn-outline" onClick={ping}><Navigation size={15} /> Share location</button>
          {trip.next_status && (
