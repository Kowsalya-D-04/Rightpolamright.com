...
const RATE_FIELDS = [
  ['base_fare', 'Base Fare'], ['rate_per_km', 'Per KM'], ['rate_per_ton', 'Per Ton'], ['rate_per_kg', 'Per KG'], ['fuel_charge_per_km', 'Fuel / KM'],
...
export default function Pricing() {
  ...
  const [form, setForm] = useState({
    pickup_location: '', drop_location: '', weight_ton: 10, truck_type: 'Open Truck',
  })
  ...
  const saveRule = async (id) => {
    ...
    try {
      ...
      if (!changeReason.trim()) throw new Error('Enter a reason for this pricing change.')
      await api.put(`/pricing/rules/${id}`, { ...payload, reason: changeReason.trim() })
      setEditingId(null)
      setChangeReason('')
      await refreshRules()
      if (quote) calculate()
  ...
  const lines = quote && [
    ...
    ['Weight charge', quote.weight_charge], ['Fuel charge', quote.fuel_charge], ['Toll charges', quote.toll_charge],
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div className="table-wrap">
          <table className="data">
            <thead><tr><th>Truck Type</th><th>Base Fare</th><th>Per KM</th><th>Per Ton</th><th>Per KG</th>
                    ...
                    {RATE_FIELDS.map(([k]) => (
                      <td className="cell-mono" key={k}>
                        {editing ? (
                          ...
                          : (k === 'rate_per_km' || k === 'rate_per_ton' || k === 'rate_per_kg') ? `₹${r[k]}`
                          : money(r[k])}
                      </td>
                    ))}
                    <td>
                      {editing ? (
                        <div>
                          <input className="input" placeholder="Reason for change" value={changeReason} onChange={(e)=>setChangeReason(e.target.value)} style={{minWidth:160,marginBottom:6}} />
